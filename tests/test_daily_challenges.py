...
def test_daily_challenges_render(client):
    ...
    for challenge in DAILY_CHALLENGES[:3]:  # Assuming 3 daily challenges are displayed
...
def test_mark_challenge_completed(client):
    ...
    response = client.post(url_for('daily_challenge'), data={"completed_challenge": challenge_to_complete})
...
def test_shareable_message_when_all_challenges_completed(client):
    ...
    today = datetime.today().strftime('%Y-%m-%d')
    challenges = DAILY_CHALLENGES[:3]  # Assuming the first 3 are displayed as daily challenges

    # Mark all challenges as completed
    for challenge in challenges:
        client.post(url_for('daily_challenge'), data={"completed_challenge": challenge})
